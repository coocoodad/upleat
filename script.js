...
window.addEventListener('load', () => {
    ...
    const upleatCurtains = new Curtains({
        container: "canvas",
        ...
        fonts.list.forEach(font => {
            document.fonts.load(font).then(() => {
                ...
                if(fonts.loaded === fonts.list.length) {
                    ...
                    const scrollPass = new ShaderPass(upleatCurtains, {
                    ...
                    scrollPass.onRender(() => {
                        ...
                        scroll.value = upleatCurtains.getScrollValues().y;

                        // clamp delta
                        scroll.delta = Math.max(-30, Math.min(30, scroll.lastValue - scroll.value));

                        scroll.effect = upleatCurtains.lerp(scroll.effect, scroll.delta, 0.05);
                    ...
                    textEls.forEach(textEl => {
                        const textPlane = new Plane(upleatCurtains, textEl, {
                            vertexShader: vs,
                            fragmentShader: fs
                        });
